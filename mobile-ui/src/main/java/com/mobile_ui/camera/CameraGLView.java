...
import com.v9kmedia.v9krecorder.utils.GLDrawer2D;
...
public class CameraGLView extends GLSurfaceView {
	...
	public CameraGLView(final Context context, final AttributeSet attrs, final int defStyle) {
		super(context, attrs);
		 if(DEBUG) Log.d(TAG, "CameraGLView:");
...
/*		// the frequency of refreshing of camera preview is at most 15 fps
	...
	public void onResume() {
		...
		if (mHasSurface) {
			if (mCameraHandler == null) {
				 if(DEBUG) Log.d(TAG, "surface already exist");
    ...
    public void switchCamera() {
        if(DEBUG) Log.d(TAG, "inPreview: " + inPreview);

        if(DEBUG) Log.d(TAG, "mCameraHandler: " + mCameraHandler);

        if(inPreview) {
            if(mCameraHandler != null) {
                // stop preview before switching camera
                if(DEBUG) Log.d(TAG, "stopping preview before switching camera");
                mCameraHandler.stopPreview(true);
                mCameraHandler.switchCameraPreview();
            }
        }
	...
	public SurfaceTexture getSurfaceTexture() {
		 if(DEBUG) Log.d(TAG, "getSurfaceTexture:");
	...
	public void setVideoEncoder(final MediaVideoEncoder encoder) {
        ...
        if(DEBUG) Log.d(TAG, "setVideoEncoder:tex_id= " + mRenderer.hTex + ",encoder=" + encoder);
    ...
    public void updateEGLContext(final MediaVideoEncoder encoder) {
        queueEvent(new Runnable() {
            @Override
            public void run() {
                synchronized (mRenderer) {
                    encoder.updateSharedContext(EGL14.eglGetCurrentContext(), mRenderer.hTex);
                }
            }
        });
    }
    ...
    public void changePauseState(boolean isPause) {
        mRenderer.changePauseState(isPause);
    }
	...
	private static final class CameraSurfaceRenderer
        ...
        private volatile boolean pauseEnabled = false;
		private int hTex;
		...
		public CameraSurfaceRenderer(final CameraGLView parent) {
			if(DEBUG) Log.d(TAG, "CameraSurfaceRenderer:");
...
//			if(DEBUG) Log.d(TAG, "onSurfaceCreated:Gl extensions: " + extensions);
		...
		public void onSurfaceChanged(final GL10 unused, final int width, final int height) {
			if(DEBUG) Log.d(TAG, String.format("onSurfaceChanged:(%d,%d)", width, height));
		...
		private final void updateViewport() {
			...
			if (parent != null) {
				...
				if(DEBUG) Log.d(TAG, String.format("view(%d,%d)%f,video(%1.0f,%1.0f)", view_width, view_height, view_aspect, video_width, video_height));
				...
				switch (parent.mScaleMode) {
                    ...
                    case SCALE_KEEP_ASPECT_VIEWPORT:
                    {
                        ...
                        if(DEBUG) Log.d(TAG, String.format("xy(%d,%d),size(%d,%d)", x, y, width, height));
                    ...
                    case SCALE_CROP_CENTER:
                    {
                        ...
                        if(DEBUG) Log.d(TAG, String.format("size(%1.0f,%1.0f),scale(%f,%f),mat(%f,%f)",
...
//						mVideoEncoder.frameAvailableSoon(mStMatrix);
                        if(!pauseEnabled) {
                            if(DEBUG) Log.d(TAG, "pause not enabled...");
                            mVideoEncoder.frameAvailableSoon(mSTexture, mStMatrix, mMvpMatrix);
                        } else {
                            if(DEBUG) Log.d(TAG, "pause enabled...");
