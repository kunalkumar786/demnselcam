...
public final class RenderHandler implements Runnable {
	...
	public static final RenderHandler createHandler(final String name) {
		 if(DEBUG) Log.d(TAG, "createHandler:");
	...
	public final void draw(final float[] tex_matrix, final float[] mvp_matrix, final long pt) {
...
//********************************************************************************
	...
	public final void run() {
		 if(DEBUG) Log.d(TAG, "RenderHandler thread started:");
		synchronized (mSync) {
...
//					mSync.notifyAll();
        	...
        	if (localRequestDraw) {
        		if ((mEgl != null) && mTexId >= 0) {
                    ...
                    mInputSurface.setPresentationTime(presentation_time);
            		mInputSurface.swap();
        		}
        	} else {
        		synchronized(mSync) {
        ...
        synchronized (mSync) {
		 ...
		 if(DEBUG) Log.d(TAG, "RenderHandler thread finished:");
	}

	private final void internalPrepare() {
		 if(DEBUG) Log.d(TAG, "internalPrepare:");
		internalRelease();
