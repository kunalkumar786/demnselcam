...
public class V9krecorderutil {

	public static ContentValues videoContentValues = null;

    public static HashMap<String, String> mediaMap = new HashMap<>();

    private static final SimpleDateFormat mDateTimeFormat = new SimpleDateFormat("yyyy_MM_dd_HH_mm_ss_SSSZ", Locale.US);
	...
	public static String createImagePath(Context context)
    {
		...
		String title = Constant.FILE_START_NAME + dateTaken;
		String filename = title + Constant.IMAGE_EXTENSION;
	...
	public static String createFinalPath(Context context)
	{
		long dateTaken = System.currentTimeMillis();
		String title = Constant.FILE_START_NAME + dateTaken;
		String filename = title + Constant.VIDEO_EXTENSION;
		String filePath = generateFilePath(context, String.valueOf(dateTaken), true, null);
    ...
    public static String createStreamPath(String type, String stream) {

		String fileName = Constant.FILE_START_NAME + String.valueOf(System.currentTimeMillis());

		final File dir = new File(Environment.getExternalStoragePublicDirectory(type), Constant.DIR_NAME);
        ...
        if(dir.canWrite()) {
            switch(stream) {
                case "video":
                    final String videoPath = new File(dir, fileName + Constant.VIDEO_EXTENSION).toString();
                    mediaMap.put("video", videoPath);
                    return videoPath;
                case "audio":
                    final String audioPath = new File(dir, fileName + Constant.MP4_AUDIO_EXTENSION).toString();
                    mediaMap.put("audio", audioPath);
                    return audioPath;
                case "trimaudio":
                    final String trimAudioPath = new File(dir, fileName + Constant.MP3_AUDIO_EXTENSION).toString();
                    mediaMap.put("trimaudio", trimAudioPath);
                    return trimAudioPath;
                case "movie":
                    final String moviePath = new File(dir, fileName + Constant.VIDEO_EXTENSION).toString();
                    mediaMap.put("movie", moviePath);
                    return moviePath;
            }
        }
	...
	private static String generateFilePath(Context context, String uniqueId, boolean isFinalPath, File tempFolderPath)
	{
		String fileName = Constant.FILE_START_NAME + uniqueId + Constant.VIDEO_EXTENSION;
	...
	{
		long dateTaken = System.currentTimeMillis();
		String filePath = generateFilePath(context, String.valueOf(dateTaken), false, tempFolderPath);
		return filePath;
	}

	public static File getTempFolderPath()
	{
		File tempFolder = new File(Constant.TEMP_FOLDER_PATH + "_" + System.currentTimeMillis());
		return tempFolder;
	}
	...
	{
		int margin = 0;
		if(previewWidth <= Constant.RESOLUTION_LOW)
		{
			margin = (int) (screenWidth*0.12);
		}
		else if(previewWidth > Constant.RESOLUTION_LOW && previewWidth <= Constant.RESOLUTION_MEDIUM)
		{
			margin = (int) (screenWidth*0.08);
		}
		else if(previewWidth > Constant.RESOLUTION_MEDIUM && previewWidth <= Constant.RESOLUTION_HIGH)
		{
			margin = (int) (screenWidth*0.08);
		}
		return margin;
	...
	public static int setSelectedResolution(int previewHeight)
	{
		...
		if(previewHeight <= Constant.RESOLUTION_LOW)
		{
			selectedResolution = 0;
		}
		else if(previewHeight > Constant.RESOLUTION_LOW && previewHeight <= Constant.RESOLUTION_MEDIUM)
		{
			selectedResolution = 1;
		}
		else if(previewHeight > Constant.RESOLUTION_MEDIUM && previewHeight <= Constant.RESOLUTION_HIGH)
		{
