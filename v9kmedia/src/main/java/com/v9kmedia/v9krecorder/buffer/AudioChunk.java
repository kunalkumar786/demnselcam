...
public class AudioChunk extends DataChunk
{

	private static final String TAG = "AudioChunks";
     ...
     */
    public AudioChunk(byte bitPerSample, int sampleRate, byte channel, int desiredSpanSec) {
        ...
        int dataBufferSize = ((bitPerSample * sampleRate * channel * desiredSpanSec) / 8);
        ...
        int metaBufferCount = sampleRate * desiredSpanSec * 2;
        ...
        if (VERBOSE) {
            if(DEBUG) Log.d(TAG, "CBE: bitRate=" + (bitPerSample * sampleRate) + " sampleRate=" + sampleRate +
